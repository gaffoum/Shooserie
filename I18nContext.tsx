...
export function Dashboard() {
  ...
  // When a barcode scan resolves to a product that's already in the user's
  // collection, we pause the navigation and surface a confirmation dialog so
  // the user can choose to add anyway (legitimate duplicate copies) or back
  // out. Holds the defaults that would be passed to /sneakers/new and the
  // list of matching existing sneakers used to populate the dialog body.
  const [pendingDuplicate, setPendingDuplicate] = useState<{
    defaults: Record<string, unknown>
    state: { defaults: Record<string, unknown>; scannedFrom: string; lookupSource: string | null }
    matches: Sneaker[]
  } | null>(null)
  ...
  const handleDashboardScan = (result: ScanResult) => {
    ...
    // Duplicate detection — match by (in priority order):
    //   1. stockx_product_id (most reliable: same model in any size)
    //   2. sku (case-insensitive, trimmed)
    //   3. barcode (exact)
    // If anything matches, pause the navigation and ask the user whether
    // they really want to add a duplicate.
    const matches = findDuplicates(allSneakers, result, defaults)
    if (matches.length > 0) {
      setPendingDuplicate({ defaults, state: navState, matches })
      return
    }

    navigate('/sneakers/new', { state: navState })
  }

  /** Confirms the pending duplicate add — navigate to the new-sneaker form. */
  const confirmDuplicateAdd = () => {
    if (!pendingDuplicate) return
    const navState = pendingDuplicate.state
...
/**
 * Find existing sneakers in the user's collection that look like duplicates
 * of a scan result. Three-tier matching, most reliable first:
 *
 *   1. `stockx_product_id` — same model (most reliable; different sizes of
 *      the same shoe still count as the same model)
 *   2. `sku` — case-insensitive, trimmed
 *   3. `barcode` — exact match
 *
 * Returns all matches found (deduped by sneaker id). The caller decides what
 * to show — typically a confirmation dialog letting the user add anyway if
 * they really own multiple copies.
 */
function findDuplicates(
  ...
  // We also use the SKU we deduced into `defaults` (sometimes the scanned
  // code IS the SKU, sometimes it's a UPC and we look up the SKU separately).
  const sku =
    typeof defaults.sku === 'string'
      ? defaults.sku.trim().toUpperCase()
      : null
